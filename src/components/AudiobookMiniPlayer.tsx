import React, { memo } from 'react'
import { useShallow } from 'zustand/shallow'
import { Play, Pause, RotateCcw, RotateCw, BookAudio, X } from 'lucide-react'
import { useAudiobookPlayer } from '../stores/audiobookPlayerStore'
import { formatAbsTime } from './audiobookshelf/absHelpers'

interface AudiobookMiniPlayerProps {
  onSwitchToAudiobooks: () => void
}

// Compact audiobook bar shown at the top of non-Audiobookshelf tabs while an
// audiobook/podcast is loaded (same pattern as MusicMiniPlayer).
export const AudiobookMiniPlayer = memo(function AudiobookMiniPlayer({ onSwitchToAudiobooks }: AudiobookMiniPlayerProps) {
  // Shows a progress bar, so currentTime is needed (4x/sec while playing);
  // useShallow still skips re-renders from unrelated store churn.
  const {
    currentItem,
    displayTitle,
    displayAuthor,
    coverUrl,
    isPlaying,
    currentTime,
    duration,
    toggle,
    seekRelative,
    seek,
    stop,
  } = useAudiobookPlayer(useShallow(s => ({
    currentItem: s.currentItem,
    displayTitle: s.displayTitle,
    displayAuthor: s.displayAuthor,
    coverUrl: s.coverUrl,
    isPlaying: s.isPlaying,
    currentTime: s.currentTime,
    duration: s.duration,
    toggle: s.toggle,
    seekRelative: s.seekRelative,
    seek: s.seek,
    stop: s.stop,
  })))

  if (!currentItem) return null

  const progressPercent = duration > 0 ? (currentTime / duration) * 100 : 0

  const handleProgressClick = (e: React.MouseEvent<HTMLDivElement>) => {
    const rect = e.currentTarget.getBoundingClientRect()
    const pos = (e.clientX - rect.left) / rect.width
    seek(pos * duration)
  }

  return (
    <div className="flex-shrink-0 h-12 bg-black/80 backdrop-blur-md border-b phone:border-b-0 phone:border-t border-white/10 flex items-center px-4 gap-3">
      {/* Book info - clickable to go to audiobooks view */}
      <button
        onClick={onSwitchToAudiobooks}
        className="flex items-center gap-2 min-w-0 flex-shrink hover:opacity-80 transition-opacity"
        title="Open Audiobooks"
      >
        <div className="w-7 h-7 flex-shrink-0 rounded bg-white/10 flex items-center justify-center overflow-hidden">
          {coverUrl ? (
            <img
              src={coverUrl}
              alt={displayTitle}
              className="w-full h-full object-cover"
            />
          ) : (
            <BookAudio size={12} className="text-white/40" />
          )}
        </div>
        <div className="min-w-0 hidden sm:block phone:block text-left">
          <p className="text-xs font-medium text-white truncate max-w-[140px]">{displayTitle}</p>
          {displayAuthor && <p className="text-[10px] text-white/50 truncate max-w-[140px]">{displayAuthor}</p>}
        </div>
      </button>

      {/* Controls */}
      <div className="flex items-center gap-1">
        <button
          onClick={() => seekRelative(-30)}
          className="p-1.5 text-white/60 hover:text-white transition-colors"
          title="Back 30 seconds"
        >
          <RotateCcw size={14} />
        </button>

        <button
          onClick={toggle}
          className="p-1.5 bg-white rounded-full text-black hover:scale-105 transition-transform"
          title={isPlaying ? 'Pause' : 'Play'}
        >
          {isPlaying ? (
            <Pause size={14} fill="currentColor" />
          ) : (
            <Play size={14} fill="currentColor" className="ml-px" />
          )}
        </button>

        <button
          onClick={() => seekRelative(30)}
          className="p-1.5 text-white/60 hover:text-white transition-colors"
          title="Forward 30 seconds"
        >
          <RotateCw size={14} />
        </button>
      </div>

      {/* Progress */}
      <div className="flex-1 flex items-center gap-2 min-w-0">
        <span className="text-[10px] text-white/40 tabular-nums flex-shrink-0">{formatAbsTime(currentTime)}</span>
        <div
          className="flex-1 h-1 bg-white/20 rounded-full cursor-pointer group"
          onClick={handleProgressClick}
        >
          <div
            className="h-full bg-theme-400 group-hover:bg-theme-300 rounded-full transition-colors"
            style={{ width: `${progressPercent}%` }}
          />
        </div>
        <span className="text-[10px] text-white/40 tabular-nums flex-shrink-0">{formatAbsTime(duration)}</span>
      </div>

      {/* Stop */}
      <button
        onClick={stop}
        className="flex-shrink-0 p-1.5 text-white/40 hover:text-white transition-colors"
        title="Stop playback"
      >
        <X size={14} />
      </button>
    </div>
  )
})
